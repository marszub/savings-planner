...
import { CreateCyclicEventRequest, CreateOneTimeEventRequest } from "../requests/create-event-request";
...
export const eventService = {
    ...
    update(id, cyclic, formModel) {
      ...
      return httpService.put(`/events/${id}`, body)
          .then(async res => {
            switch (res.status) {
              case HTTP_NO_CONTENT:
                this._replace_event(id, res.body);
                this._events.sort(compare);
                this._notifyChangeListeners();
                break;
              case HTTP_NOT_FOUND:
