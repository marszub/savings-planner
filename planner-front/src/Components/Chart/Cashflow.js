      ...
      afterDraw(chart, args, pluginOptions) {
        ...
        class CircleChevron {
          draw(ctx, x1, pixel) {
            ...
            ctx.arc(x1, height / 2 + top, 15, 0, angle * 360, false);
            ctx.stroke();
            ctx.fill();
            ctx.closePath();
  ...
  const createCashflow = (events, balance) => {
    ...
    while (i < events.length) {
      ...
      if (datePrev > new Date()) {
        ...
        while (
          datePrev.getFullYear() == dateCurr.getFullYear() &&
          datePrev.getMonth() == dateCurr.getMonth() &&
          datePrev.getDate() == dateCurr.getDate() &&
          i + 1 < events.length
        ) {
        ...
        if (j == -1) cashData.push((balance + amountOneDate) / 100);
        else cashData.push(cashData[j] + amountOneDate / 100);
        j++;
      }
      i++;
    }
